...
def main():
    ...
    y_strat = strat_df["IsRecommended"].reset_index(drop=True)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv.split(df_valid, y_strat), start=1):
        ...
        val_scored = val_scored.dropna(subset=["FinalScore"]).copy()

        fold_threshold = float(train_scored["FinalScore"].quantile(0.75))
        ...
        y_train = train_labeled["IsRecommended"].copy()

        X_val = val_labeled[MODEL_FEATURES].copy()
        y_val = val_labeled["IsRecommended"].copy()
